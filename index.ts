...
namespace CUPParser {
  ...
  export function parseWaypoints(str: string): Waypoint[] {
    ...
    }).filter(row => 'name' in row) as Waypoint[];
  }

  export function parseWaypointStyle(str: string | undefined): WaypointStyle {
    if (!str) return WaypointStyle.Unknown;

    let value = Number(str);
    return value < 1 || value > 17 ? WaypointStyle.Unknown : value;
  }

  export function parseTasks(str: string): Task[] {
    if (!str) return [];
    ...
    for (let row of rows) {
      if (row[0] === 'Options') {
        ...
        let points = names.map(name => ({ name } as Taskpoint));
  ...
  export function parseTaskOptions(columns: string[]): TaskOptions {
    ...
    for (let column of columns) {
      ...
      if (name === 'NoStart') {
        ...
        options.taskTime = parseDuration(value);

      } else if (name === 'WpDis') {
        options.wpDis = parseBoolean(value);
        ...
        options.minDis = parseBoolean(value);

      } else if (name === 'RandomOrder') {
        options.randomOrder = parseBoolean(value);
  ...
  export function parseTaskpointOptions(columns: string[]): TaskpointOptions {
    let options: TaskpointOptions = { line: false };
